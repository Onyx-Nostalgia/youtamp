...
## Key Features
**Responsive Web Design:** Ensures an optimal and aesthetic viewing experience across all devices (mobile phones, tablets, and desktops).

**Integrated Copy-to-Clipboard:** A dedicated button to easily copy the generated timestamps for immediate external use (e.g., pasting into YouTube comments).

**Multilingual Support (TH/EN):** The website supports toggling between Thai and English languages.

**Emphasis on Motion and Animation:** The user interface should feature engaging **animations and transitions** to enhance the user experience. (use Tawilwind CSS and tailwindcss-animated plugin:
 https://github.com/new-data-services/tailwindcss-animated/blob/2.x/README.md)

**Sticky Video Player:** The video player remains visible at the top of the viewport as users scroll through the timestamp list, ensuring continuous access to video playback controls.

**Timestamp Display/Edit Section:**
- Features a **Toggle Button** to switch between Display Mode and Edit Mode.
- Includes a dedicated **Copy Button**.
- The timestamp content area must be implemented as an **Internal Scrolling Area**, allowing users to scroll the results list without moving the entire page or obscuring the sticky elements (Video Player, Copy Button).

# Base Frontend Structure Rule

**HTML Directory:** `./templates/`
- Main HTML File: `./templates/index.html`

**Static Assets Directory:** All required assets (CSS, JS, images, etc.) are located in `./static/`.

**CSS Workflow:**
- **Source CSS:** `./static/css/input.css` (Note: The project uses a build system to transform `input.css` into the production file, e.g., `dist/output.css`, which is referenced by the HTML).

**JavaScript Implementation:**
- **Source JS:** `./static/app.js`


# Base Font Design
- Use font-family 'Noto Sans Thai Looped' 
...
```css
body {
    --font-sans: 'Noto Sans Thai Looped', sans-serif;
}
```
...
Based on **daisyUI 5 (Priority 1)** And **Tailwind CSS v4 (Secondary priority)**
